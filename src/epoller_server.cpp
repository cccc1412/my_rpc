...
EpollerServer::~EpollerServer() { delete net_threads_; }

void EpollerServer::Send(unsigned int uid, const string &s, const string &ip,
                         uint16_t port) {
...
void NetThread::parseAddr(const string &addr_string, in_addr &addr_struct) {
  int ret = inet_pton(AF_INET, addr_string.c_str(),
                      &addr_struct);  //点分十进制转化为二进制整数
  if (ret < 0) {
...
int NetThread::bind(string &ip, int &port) {
  ...
  int ret = ::bind(sock_fd_, (sockaddr *)&bind_addr,
                   sizeof(bind_addr));  //::是全局作用域符
  if (ret < 0) {
...
void NetThread::run() {
  while (1) {
    ...
    for (int i = 0; i < ev_num; i++) {
      ...
      uint32_t h = e.data.u64 >> 32;
      // cout<<"fd:"<<e.data.fd<<endl;
      if (h == ET_LISTEN) {
        int client_fd = accept(sock_fd_);
        if (client_fd <= 0) continue;
        // epoll_event ev;
        // ev.data.u64 = client_fd;
        epoller_.add(client_fd, client_fd, EPOLLIN | EPOLLET);
      } else if (h == ET_NOTIFY) {
...
void NetThread::createEpoll() {  //创建epoll，对三个文件描述符添加事件监听
  ...
  epoller_.add(sock_fd_, H64(ET_LISTEN) | sock_fd_,
               EPOLLIN);  // EPOLLIN 表示有新数据到达，socket可读
  // epoller_.add(shutdown_sock_fd_, NULL, EPOLLIN);
  epoller_.add(notify_sock_fd_, H64(ET_NOTIFY), EPOLLIN | EPOLLET);
}

void NetThread::send(uint32_t uid, const string &s, const string &ip,
                     uint16_t port) {
  ...
  epoller_.mod(notify_sock_fd_, H64(ET_NOTIFY), EPOLLOUT | EPOLLET);
}
...
void Handle::SendResponse(uint32_t uid, const string &s, const string &ip,
                          int port) {
...
NetThread *EpollerServer::GetNetThread() { return net_threads_; }
void Handle::Run() {
  initalize();
  HandleImp();
}
...
void Handle::HandleImp() {
  ...
  while (true) {
    ...
    while (WaitForRecvQueue(recv, 0)) {
      ...
      // compute
      req.resize(recv->buffer.size());
      req.assign(recv->buffer.begin(), recv->buffer.end());
      int ret = p_epoll_server_->sm.serve(req, res);
      string send_str(res.begin(), res.end());
      p_epoll_server_->Send(recv->fd, send_str, "0", 0);
    }
  }
}
